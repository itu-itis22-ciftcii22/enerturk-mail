...
class MailFile():
    def __init__(self,  username : string, folder : string, mail : EmailMessage):
        self.username = username
        self.mail = mail
        self.mail_id = mail.get("Message-ID") or f"<{uuid.uuid4()}@enerturkmail>"
        self.folder = folder

    def save(self, base_dir : string):
    ...
    @classmethod
    def load_from_file(cls, file_path : string, username : string, folder : string)
        with open(file_path, "rb") as mail_file:
            mail = BytesParser(policy=default).parse(file_name)
